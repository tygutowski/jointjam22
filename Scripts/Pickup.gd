...
func body_entered(body):
	if body.name == "Player":
		pick_item()
		empower_player(body)
		get_tree().get_nodes_in_group("Level")[0].get_node("AudioStreamPlayer").playing = true
		var particle = load("res://Pickups/PickupParticles.tscn")
		var new_particle = particle.instance()
		get_tree().get_nodes_in_group("Level")[0].get_node("Particles").add_child(new_particle)
