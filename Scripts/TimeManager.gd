...
func level_selected(level_num, collected_all_pickups):
	var err = get_tree().change_scene("res://Levels/Level" + str(level_num) + "/Level" + str(level_num) + ".tscn")
	var loading = load("res://Levels/LoadingScreen.tscn")
	var screen = loading.instance()
	add_child(screen)
	if err == OK:
		...
		if !collected_all_pickups:
			get_node("/root/Level" + str(current_level) + "/CanvasLayer/Label").visible = true
			yield(get_tree().create_timer(2.0), "timeout")
			get_node("/root/Level" + str(current_level) + "/CanvasLayer/Label").visible = false
	screen.queue_free()
